...
```
Таким образом обновляя глобальные переменные **filVal_x**, **filVal_y** и **filVal_z**. Далее эти переменные используются для получения целевых значений.
...
Пайка контактов может осущетсвляться жалом с температурой до 450 град С. 
...
![Схема отладки](https://github.com/Davidovskii-Nikita/vibroanaliz/blob/master/docs/C%D1%85%D0%B5%D0%BC%D0%B0%20%D0%BF%D1%80%D0%BE%D1%88%D0%B8%D0%B2%D0%BA%D0%B8.png)
...
Используя функцию *get_value_from_reg* можно получить значения ускорения, испытываемого на осях датчика. Перемещая устройство так, что бы сила притяжения была параллельна каждой оси со и против направления движения собрать полученные значения. Они должны быть примерно равны 1 в случае противонаправленности и -1 в случае сонаправленности. Если значения отличаются от 1, тогда можно изменить следующие переменные:
``` C
...
---
### Возможные проблемы.
* Редкий баг с получением неправильного времени.
Появлятеся в случае частого перепрошивания ESP через браузер.  

Возможное решение - поменять библиотеку *NTPClient.h* и поставить защиту от записи такого времени ( простой *if* с перезарузкой *ESP.restart()*)

* Механизм нивелирования постоянной силы притяжения.
Требуется практическая проверка.  

Возможное решение - использовать гироскоп и/или комплиментарный фильтр для получения информации о положении в пространстве с дальнейшей фильтрацией получаемых с осей акселерометра даннымх.

* Обработка получаемых с осей акселерометра значений ускорений. Требуется практическая проверка.   

Возможное решение - пересмотреть метод интегрирования (другая формула). Добавить фильтры (мода, ФНЧ и тд.).

* Залитая через OTA - обновление прошивка может быть "не принята" контроллером. В этом случае стоит повторить обновление.    

Возможное решение - использовать библиотеку для OTA - обновлений.
...
* Добавить возможность прошивать устройство через сервер. В документации есть про это [статья](https://arduino-esp8266.readthedocs.io/en/2.7.2/ota_updates/readme.html#http-server). 

* Уменьшить энергопотребление модуля.     
Основной потребитель электричества - WiFi. Если OTA-обновления будут более контролируемыми, можно отключать вай-фай (Wifi.Mode(Wifi_Off)), использовать режим глубокого сна ([ESP.deepSleep()](http://wikihandbk.com/wiki/ESP8266:%D0%9F%D1%80%D0%B8%D0%BC%D0%B5%D1%80%D1%8B/%D0%98%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5_ESP8266_%D0%B2_%D1%80%D0%B5%D0%B6%D0%B8%D0%BC%D0%B5_%D0%B3%D0%BB%D1%83%D0%B1%D0%BE%D0%BA%D0%BE%D0%B3%D0%BE_%D1%81%D0%BD%D0%B0_%D0%BF%D1%80%D0%B8_%D0%BF%D0%BE%D0%BC%D0%BE%D1%89%D0%B8_IDE_Arduino) и [припаять ножку МК](https://blog.enbiso.com/post/esp-01-deep-sleep/)).
Так же про это есть [статья](https://habr.com/ru/post/480316/).
