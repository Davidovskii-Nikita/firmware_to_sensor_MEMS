...
Однако подключение пинов esp(2, 0) к пинам MPU6050(SCL, SDA) может быть измененно программно:
```C
...
```   
Для настройки чувстительности акселерометра так же необходимо изменить передаваемую в функции константу:
```C
I2C_Write(MPU6050SlaveAddress, MPU6050_REGISTER_ACCEL_CONFIG, 0x18);
```
0x18 - для +-16g; 0x10 - для +-10g; 0x08 - для +- 4g; 0x00 для 2g.
...
* Во время работы, ESP разоревается до 35 °С.
